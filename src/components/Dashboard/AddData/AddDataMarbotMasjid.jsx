...
const AddDataMarbotMasjid = ({ title }) => {
  const [NAMA_TEMPAT_IBADAH, setNamaTempat] = useState("");
  ...
  const handleSave = async (e) => {
    ...
    try {
      await axios
        .post(`http://127.0.0.1:3000/members/saveMarbotMasjid`, {
          KECAMATAN,
          KELURAHAN,
          NAMA_TEMPAT_IBADAH,
          RT,
  ...
  useEffect(() => {
    if (KECAMATAN && selectedOption[KECAMATAN] && selectedOption[KECAMATAN].length > 0) {
      setKelurahan(selectedOption[KECAMATAN][0]);
    }
  }, [KECAMATAN, selectedOption]);
  return (
    <div>
      <h1 className="font-bold text-xl text-center m-3">Tambah Data {title}</h1>
      <form className="w-11/12 lg:w-full h-full px-10 flex flex-row" onSubmit={handleSave}>
        <div className="flex flex-col w-full h-full gap-7">
          <div>
            <div className="space-y-7 h-5/6 flex flex-col">
              ...
              <div className="flex flex-col">
                <label htmlFor="inputNama">Nama Petugas</label>
                <input
                  type="text"
                  id="inputNama"
                  value={NAMA_PETUGAS}
              ...
              <div className="flex flex-col">
                <label htmlFor="inputJamaah">Nama Tempat Ibadah</label>
                <input
                  type="text"
                  id="inputJamaah"
...
export default AddDataMarbotMasjid;
