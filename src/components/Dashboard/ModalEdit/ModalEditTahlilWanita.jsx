...
const ModalEditTahlilWanita = ({
  ...
  const [NAMA_JAMAAH_TAHLIL, setNamaJamaah] = useState(data.NAMA_JAMAAH_TAHLIL);
  ...
  const [DIGIT, setDigit] = useState(data.DIGIT);
  const [curr, setCurr] = useState(0);
  const getById = async (e) => {
    const res = await axios.post(
      `http://127.0.0.1:3000/members/jmtWanita/download`,
      checkedIds
    );
    setDataIds(res.data);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios
        .patch(`http://127.0.0.1:3000/members/jmtWanita/edit/${id}`, {
          KECAMATAN,
          DESA,
          DUSUN,
          RT,
  ...
  const handleUpdateMany = async (e) => {
    ...
    try {
      await axios
        .put(`http://127.0.0.1:3000/members/jmtWanita/editMany`, dataIds)
        .then((res) => {
          if (res.data.status) {
            console.log("succesfully");
            allData;
          ...
          <form onSubmit={handleUpdate} className="flex">
            <Modal.Body>
              <div className="space-y-2 max-h-full">
                ...
                <div className="flex flex-col">
                  <label htmlFor="inputNamaJamaah">Nama Jamaah</label>
                  <input
                    type="text"
                    id="inputNamaJamaah"
                    value={NAMA_JAMAAH_TAHLIL}
                ...
                <div className="flex flex-row w-full">
                  ...
                  <div className="flex flex-col flex-1">
                    <label htmlFor="inputDesa">Desa</label>
                    <input
                      type="text"
                      id="inputDesa"
                      value={DESA}
                ...
                <div className="flex flex-col">
                  <label htmlFor="inputJumAnggota">Jumlah Anggota</label>
                  <input
                    type="number"
                    id="inputJumAnggota"
                ...
                <div className="flex flex-col">
                  <label htmlFor="inputDigit">Digit</label>
                  <input
                    type="number"
                    id="inputDigit"
                    value={DIGIT}
...
export default ModalEditTahlilWanita;
