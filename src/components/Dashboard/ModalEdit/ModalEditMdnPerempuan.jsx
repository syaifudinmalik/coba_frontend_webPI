...
const ModalEditMdnPerempuan = ({
  ...
  const [NAMA, setNamaKetua] = useState(data.NAMA);
  ...
  const getById = async (e) => {
    const res = await axios.post(
      `http://127.0.0.1:3000/members/mudinPerempuan/download`,
      checkedIds
    );
    setDataIds(res.data);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios
        .patch(`http://127.0.0.1:3000/members/mudinPerempuan/edit/${id}`, {
          KECAMATAN,
          KELURAHAN,
          RT,
          RW,
  ...
  const handleUpdateMany = async (e) => {
    ...
    try {
      await axios
        .put(`http://127.0.0.1:3000/members/mudinPerempuan/editMany`, dataIds)
        .then((res) => {
          if (res.data.status) {
            console.log("succesfully");
            allData;
...
export default ModalEditMdnPerempuan;
