...
const ModalEditGuruK = ({
  ...
  const [NAMA_LEMBAGA, setNamaJamaah] = useState(data.NAMA_LEMBAGA);
  ...
  const getById = async (e) => {
    const res = await axios.post(
      `http://127.0.0.1:3000/members/guruKeagamaan/download`,
      checkedIds
    );
    setDataIds(res.data);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios
        .patch(`http://127.0.0.1:3000/members/guruKeagamaan/edit/${id}`, {
          KECAMATAN,
          KELURAHAN,
          DUSUN,
          RT,
  ...
  const handleUpdateMany = async (e) => {
    ...
    try {
      await axios
        .put(`http://127.0.0.1:3000/members/guruKeagamaan/editMany`, dataIds)
        .then((res) => {
          if (res.data.status) {
            console.log("succesfully");
            allData;
