...
const ModalEditMbtMasjid = ({
  ...
  const [NAMA_TEMPAT_IBADAH, setNamaJamaah] = useState(data.NAMA_TEMPAT_IBADAH);
  const [NAMA_PETUGAS, setNamaKetua] = useState(data.NAMA_PETUGAS);
  ...
  const getById = async (e) => {
    const res = await axios.post(
      `http://127.0.0.1:3000/members/marbotMasjid/download`,
      checkedIds
    );
    setDataIds(res.data);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios
        .patch(`http://127.0.0.1:3000/members/marbotMasjid/edit/${id}`, {
          KECAMATAN,
          KELURAHAN,
          NAMA_TEMPAT_IBADAH,
          RT,
  ...
  const handleUpdateMany = async (e) => {
    ...
    try {
      await axios
        .put(`http://127.0.0.1:3000/members/marbotMasjid/editMany`, dataIds)
        .then((res) => {
          if (res.data.status) {
            console.log("succesfully");
            allData;
...
export default ModalEditMbtMasjid;
