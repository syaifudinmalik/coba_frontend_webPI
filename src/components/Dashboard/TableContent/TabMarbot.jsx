...
const TabMarbot = ({ searchDatas, onCheckbox, onLoad, loading }) => {
  ...
  const filterDataSearch = marbot.filter((item) => {
  ...
  const handleRemove = async (e) => {
    ...
    try {
      await axios.delete(
        `http://127.0.0.1:3000/members/marbotMasjid/delete/${id}`
      );
      getMembersmarbotMasjid();
    } catch (error) {
      console.log(error);
    }
    toggleModalDel();
  ...
  const handlegetUserRemove = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://127.0.0.1:3000/members/marbotMasjid/${id}`
      );
    ...
    <div>
      ...
      {loading ? (
        ...
        <table className="w-full min-w-full text-left shadow-md rounded-md">
          ...
          <tbody>
            {dataTampil.map((mbt, index) => (
              <tr key={index} className="even:bg-blue-gray-50/50">
                <td className="px-2 text-center">
                  <input
                    type="checkbox"
                    name="inputCheck"
                    checked={checkedItems[mbt._id] || false}
                    className="rounded-sm"
                ...
                <td className="px-2 text-center">{mbt.NAMA_PETUGAS}</td>
                <td className="px-2 text-center">{mbt.NIK}</td>
                <td className="px-2 text-center">{mbt.NO_HP}</td>
                <td className="px-2 text-center">{mbt.KECAMATAN}</td>
                <td className="px-2 text-center">{mbt.KELURAHAN}</td>
                <td className="px-2 text-center">{mbt.DUSUN}</td>
                <td className="px-2 text-center">{mbt.RT}</td>
                <td className="px-2 text-center">{mbt.RW}</td>
                <td className="px-2 text-center">{mbt.NAMA_TEMPAT_IBADAH}</td>
                <td className="px-2 text-center">{mbt.REK}</td>
                <td className="p-4">
