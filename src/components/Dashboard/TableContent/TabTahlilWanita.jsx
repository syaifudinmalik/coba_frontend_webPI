...
const TabTahlilWanita = ({ searchDatas, onCheckbox, onLoad, loading }) => {
  ...
  const filterDataSearch = jmtWanita.filter((item) => {
  ...
  const handleRemove = async (e) => {
    ...
    try {
      await axios.delete(
        `http://127.0.0.1:3000/members/jmtWanita/delete/${id}`
      );
      getMembersJmtWanita();
    } catch (error) {
      console.log(error);
  ...
  const handlegetUserRemove = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://127.0.0.1:3000/members/jmtWanita/${id}`
      );
          ...
          <tbody>
            {dataTampil.map((jmt, index) => (
              <tr key={index} className="even:bg-blue-gray-50/50">
                ...
                <td className="px-2 text-center">{jmt.NAMA_JAMAAH_TAHLIL}</td>
                <td className="px-2 text-center">{jmt.REK}</td>
                <td className="px-2 text-center">{jmt.DIGIT}</td>
                <td className="px-2 text-center">
...
export default TabTahlilWanita;
