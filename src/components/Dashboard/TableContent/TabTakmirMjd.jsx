...
const TabTakmirMjd = ({ searchDatas, onCheckbox, onLoad, loading }) => {
  ...
  const filterDataSearch = tmrMasjid.filter((item) => {
  ...
  const handleRemove = async (e) => {
    ...
    try {
      await axios.delete(
        `http://127.0.0.1:3000/members/takmirMasjid/delete/${id}`
      );
      getMembersTMRMASJID();
    } catch (error) {
      console.log(error);
  ...
  const handlegetUserRemove = async (e) => {
    ...
    try {
      const res = await axios.get(
        `http://127.0.0.1:3000/members/takmirMasjid/${id}`
      );
    ...
    <div>
      ...
      {loading ? (
        ...
        <table className="w-full min-w-full text-left shadow-md rounded-md">
          ...
          <tbody>
            {dataTampil.map((tmr, index) => (
              <tr key={index} className="even:bg-blue-gray-50/50">
                <td className="px-2 text-center">
                  <input
                    type="checkbox"
                    name="inputCheck"
                    checked={checkedItems[tmr._id] || false}
                    className="rounded-sm"
                ...
                <td className="px-2 text-center">{tmr.NAMA_KETUA_TAKMIR}</td>
                <td className="px-2 text-center">{tmr.NIK}</td>
                <td className="px-2 text-center">{tmr.NO_HP}</td>
                <td className="px-2 text-center">{tmr.KECAMATAN}</td>
                <td className="px-2 text-center">{tmr.KELURAHAN}</td>
                <td className="px-2 text-center">{tmr.DUSUN}</td>
                <td className="px-2 text-center">{tmr.RT}</td>
                <td className="px-2 text-center">{tmr.RW}</td>
                <td className="px-2 text-center">{tmr.NAMA_MASJID}</td>
                <td className="px-2 text-center">{tmr.REK}</td>
                <td className="p-4">
