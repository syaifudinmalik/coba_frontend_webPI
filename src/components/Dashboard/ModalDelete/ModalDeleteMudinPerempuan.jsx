...
const ModalDeleteMudinPerempuan = ({
  tab,
  data,
  id,
  allData,
  ...
  const [NamaKetua, setNamaKetua] = useState(data.NAMA);
  ...
  const deleteMany = async (e) => {
    try {
      await axios
        .delete(`http://127.0.0.1:3000/members/mudinPerempuan/deleteMany`, {data: checkedIds})
        .then((res) => {
          onShow()
          onClear()
          allData()
...
export default ModalDeleteMudinPerempuan;
