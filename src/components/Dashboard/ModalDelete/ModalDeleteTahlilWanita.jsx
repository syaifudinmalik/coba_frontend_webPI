...
const ModalDeleteTahlilWanita = ({
  ...
  const [NamaKetua, setNamaKetua] = useState(data.NAMA_KETUA);
  ...
  const deleteMany = async (e) => {
    try {
      await axios
        .delete(`http://127.0.0.1:3000/members/jmtWanita/deleteMany`, {data: checkedIds})
        .then((res) => {
          onShow()
          onClear()
          allData()
...
export default ModalDeleteTahlilWanita;
